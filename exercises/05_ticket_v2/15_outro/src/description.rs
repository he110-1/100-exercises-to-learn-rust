...
impl TryFrom<String> for TicketDescription{
    type Error = String;
    fn try_from(data:String) -> Result<TicketDescription,String>{
        if data.is_empty(){
            ...
            return Ok(TicketDescription(data));
        }
    }
}
...
impl TryFrom<&str> for TicketDescription{
    type Error = String;
    fn try_from(data:&str) -> Result<TicketDescription,String>{
        if data.is_empty(){
            return Err("The description cannot be empty".to_string());
        }else if data.len() > 500 {
            return Err("The description cannot be longer than 500 bytes".to_string());
        }else{
            return Ok(TicketDescription(data.to_string()));
        }
    }
}
